...
class SalesDB extends Lib{
    ...
    getSales(params,filters,sorting,cb){
        if(typeof params.limit != "undefined" && params.limit > 0 &&
            ...
            super.getDB(DB).sales.find({},filters).sort(sorting).skip(skip).limit(params.limit,cb);
        }else{
            super.getDB(DB).sales.find({},filters).sort(sorting,cb);
        }
    }
    /**
    ...
    getSalesByHotelId(params,filters,sorting,cb){
        if(typeof params.limit != "undefined" && params.limit > 0
            ...
            let skip = params.offset * params.limit;
            //Tener en cuenta que utiliza la DB checkout!!!
            super.getDB(DB).sales.find({hotelId:""+params.hotelId},filters).sort(sorting).skip(skip).limit(params.limit,cb);
        }else{
            super.getDB(DB).sales.find({hotelId:""+params.hotelId},filters).sort(sorting,cb);
        }
    }
