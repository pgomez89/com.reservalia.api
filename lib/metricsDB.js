...
class MetricsDB extends Lib {
    ...
    getSalesWithBookingStatus(filters, callback) {
        var pipeline = [
            {$match: {saleOk: true}},
    ...
    getErrorsWithBookingStatus(filters, callback) {
        var pipeline = [
     ...
     * @param callback
     */
    getTotalErrors(filters, callback) {
        var pipeline = [
            {$match: {saleOk: {$exists: false}}},
            {$group: {_id: "$checkout_id", bookingList: {$push: "$booking_status"}}},
            {$match: {bookingList: {$not: {$size: 0}}}},
    ...
    getUnknownErrors(filters, callback) {
        var pipeline = [
    ...
    getTotalTokens(filters, callback) {
        var pipeline = [
